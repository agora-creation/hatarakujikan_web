                  ...
                  onPressed: () {
                    showDialog(
                      ...
                      builder: (_) => AddWorkDialog(
                        workProvider: widget.workProvider,
                        workStateProvider: widget.workStateProvider,
                        groupId: widget.sectionProvider.group?.id,
                        users: widget.sectionProvider.users,
                        user: user,
                      ),
                    );
          ...
          builder: (context, snapshot) {
            ...
            for (WorkModel _work in _worksTmp) {
              if (_work?.startedAt != _work?.endedAt) {
                ...
                _workTime = addTime(
                  _workTime,
                  _work?.workTime(widget.sectionProvider.group),
                );
                List<String> _legalTimes =
                    _work?.legalTimes(widget.sectionProvider.group);
                ...
                List<String> _nightTimes =
                    _work?.nightTimes(widget.sectionProvider.group);
